/*
 * $Id: IWSlideServiceBean.java,v 1.43.2.2 2007/03/30 07:33:31 laddi Exp $ Created on 23.10.2004
 * 
 ...
 * </p>
 * 
 * Last modified: $Date: 2007/03/30 07:33:31 $ by $Author: laddi $
 * 
...
public class IWSlideServiceBean extends IBOServiceBean implements IWSlideService, IWSlideChangeListener {
	...
	/**
	 * <p>
	 * Gets the URI for the root of the slide repository. The repository is by default mapped on '/content' under the web application.<br/> This method
	 * returns the context path for the application so if it is e.g. mapped under '/cms' this method returns '/cms/content'. If the webapplication is
	...
	/**
	 * <p>
	 * Gets the URL from with a path in the filesystem (e.g.) if the given path is '/files/public/myfile.pdf' then this method returns
	...
	/**
	 * <p>
	 * Gets the URI from with a path in the filesystem (e.g.) if the given path is '/files/public/myfile.pdf' then this method returns
	...
	/**
	 * @param scope
	 *          This parameter can be null and then the file name will be unique over the whole web. If one needs unique name within a module or a
	 *          folder one can set some (unique) string as a scope parameter
	 */
	 ...
	 * 
	 * @param path
	 *          Path with all the folders to create. Should hold all the folders after Server URI (Typically /cms/content/)
	 * @throws HttpException
	 * @throws RemoteException
	 * @throws IOException
	 ...
	 * @return true if the path refers to a folder, checks as the root user
	 */
	public boolean isFolder(String folderURI) {

		WebdavResource resource;
		try {
			resource = getWebdavResourceAuthenticatedAsRoot(folderURI);
