/*
 * $Id: IWSlideAuthenticator.java,v 1.21.2.5 2007/12/06 09:05:57 laddi Exp $
...
/**
 * <p>
 * This filter is mapped before any request to the Slide WebdavServlet to make
 * sure a logged in user from idegaWeb is logged also into the Slide
 * authentication system.
 * </p>
 * Last modified: $Date: 2007/12/06 09:05:57 $ by $Author: laddi $
 * 
...
public class IWSlideAuthenticator extends BaseFilter {
	...
	private static final String PROPERTY_ENABLED_URIS = "slide.enabled.uris";
	...
	private boolean isEnabled(HttpServletRequest request) {
		...
		if (method.equals(HTTP_METHOD_GET) || method.equals(HTTP_METHOD_POST)) {
			...
			if (requestUri.startsWith(DOCUMENTS_URI)) {
				...
				String enabledURIs = iwma.getSettings().getProperty(PROPERTY_ENABLED_URIS);
				if (enabledURIs != null) {
					StringTokenizer tokens = new StringTokenizer(enabledURIs, ",");
